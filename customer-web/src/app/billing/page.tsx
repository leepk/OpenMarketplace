...
export default function BillingPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (!off) { setData(billing ?? { orders: [], payments: [], invoices: [] }); setPackages(packs ?? []); }
  ...
  return <AccountShell user={user} title={t('paymentsInvoices')} subtitle={t('billingSubtitle')} action={<a className="primary-button" href="/post"><Icon name="plus" size={16}/> {t('upgradeListing')}</a>}>
    <div className="billing-dashboard-v4">
      ...
      <section className="billing-history-v4">
        <div className="billing-section-title-v4"><div><span>History</span><h2>Payment activity</h2></div></div>
        <div className="billing-table-v4">
          <div className="billing-table-head-v4"><span>Item</span><span>Type</span><span>Amount</span><span>Status</span><span>Action</span></div>
          {loading ? <div className="billing-empty-row-v4">Loading billing activity...</div> : null}
          {!loading && !(data.orders?.length || data.payments?.length || data.invoices?.length) ? <div className="billing-empty-row-v4">No billing activity yet.</div> : null}
